...
### Appendix: Alternative Model Outputs

This section provides outputs from fitting of basic tree, boosted, and Naive Bayes models. As seen in the summaries below, these models underperformed substantially when compared to the random forest model.

```{r}
#modelPCATree <- train(train$classe ~ .,data=trainPC, method="rpart",trControl=tc)
confusionMatrix(validation$classe,predict(modelPCATree,validationPC))

#modelPCABoost <- train(train$classe ~ .,data=trainPC, method="gbm",trControl=tc)
confusionMatrix(validation$classe,predict(modelPCABoost,validationPC))

#modelPCANB <- train(train$classe ~ .,data=trainPC, method="nb",trControl=tc)
confusionMatrix(validation$classe,predict(modelPCANB,validationPC))
```
